...
DFLASH2_Q8="$M/Qwen3.8-27B-DFlash2-z-lab-Q8_0.gguf"
DFLASH2_Q4="$M/Qwen3.8-27B-DFlash2-z-lab-Q4_K_M.gguf"
out="$root/docs/bench"
mkdir -p "$out/logs"
...
csv="$out/dflash_q4.csv"
reps_csv="$out/dflash_q4_reps.csv"
...
run() {
  ...
  local i
  for i in 1 2 3; do
    local err="$out/logs/dflash_q4_${label}_r${i}.err"
    local stdout="$out/logs/dflash_q4_${label}_r${i}.out"
...
run dflash2_q8_nm1 --spec-type draft-dflash --model-draft "$DFLASH2_Q8" -ngld 99 --spec-draft-n-max 1
run dflash2_q4_nm1 --spec-type draft-dflash --model-draft "$DFLASH2_Q4" -ngld 99 --spec-draft-n-max 1
run dflash2_q4_nm7 --spec-type draft-dflash --model-draft "$DFLASH2_Q4" -ngld 99 --spec-draft-n-max 7
